...
(function (Query) {
    ...
    function batch(value, index) {
        let start = index;
        if (utils_1.default.equals(value, index, "%24batch")) {
            index += 8;
        }
        else if (utils_1.default.equals(value, index, "$batch")) {
            index += 6;
        }
        else
            return;
        return lexer_1.default.tokenize(value, start, index, true, lexer_1.default.TokenType.Batch);
    }
    Query.batch = batch;
    ...
    function expandItem(value, index, metadataContext) {
        ...
        if (path.value.length > 1 && !path.value.some((token) => {
            return token.type !== lexer_1.default.TokenType.EntityNavigationProperty && token.type !== lexer_1.default.TokenType.EntityCollectionNavigationProperty;
        })) { // V2 Nested Navigation path
            function nestNavigation(paths) {
                let tokenValue = {};
                if (paths.length) {
                    let nav = paths.shift();
                    tokenValue.path = lexer_1.default.tokenize(value, nav.position, nav.next, [nav], lexer_1.default.TokenType.ExpandPath);
                    if (paths.length) {
                        let nestedItem = nestNavigation(paths);
                        if (nestedItem) {
                            let option = lexer_1.default.tokenize(value, nav.position, nav.next, { items: [nestedItem] }, lexer_1.default.TokenType.Expand);
                            tokenValue.options = [option];
                        }
                    }
                    return lexer_1.default.tokenize(value, nav.position, nav.next, tokenValue, lexer_1.default.TokenType.ExpandItem);
                }
                return null;
            }
            let tokenValue = nestNavigation(path.value);
            return lexer_1.default.tokenize(value, start, index, tokenValue.value, lexer_1.default.TokenType.ExpandItem);
        }
        let tokenValue = { path };
        let ref = expressions_1.default.refExpr(value, index);
        if (ref) {
    ...
    function expandPath(value, index, metadataContext) {
        ...
        if (value[index] === 0x2f) {
            ...
            if (typeName) {
                ...
                index++;
                nav = nameOrIdentifier_1.default.navigationProperty(value, index, metadataContext);
                while (nav) {
                    path.push(nav);
                    metadataContext = nav.metadata;
                    index = nav.next;
    ...
    function inlinecount(value, index) {
        ...
        else if (utils_1.default.equals(value, index, "%24inlinecount")) {
            index += 14;
            v2 = true;
        }
        else if (utils_1.default.equals(value, index, "$inlinecount")) {
            index += 12;
            v2 = true;
        }
        else
        ...
        let token;
        if (v2) {
            let equals = utils_1.default.equals(value, index, "allpages");
            let raw = true;
            if (!equals) {
                equals = utils_1.default.equals(value, index, "none");
                if (!equals) {
                    return;
                }
                raw = false;
            }
            token = lexer_1.default.tokenize(value, index, index + equals, "Edm.String", lexer_1.default.TokenType.Literal);
            token.raw = raw;
        }
        else {
            token = primitiveLiteral_1.default.booleanValue(value, index);
            if (!token)
                return;
        }
        index = token.next;
