...
(function (Query) {
    ...
    function expandItem(value, index, metadataContext) {
        ...
        if (path.value.length > 1 && !path.value.some((token) => {
            ...
            while (path.value.length > 0) {
                ...
                let nestedPath = lexer_1.default.tokenize(value, nav.position, nav.next, [nav], lexer_1.default.TokenType.ExpandPath);
                let nestedItem = lexer_1.default.tokenize(value, nav.position, nav.next, { path: nestedPath }, lexer_1.default.TokenType.ExpandItem);
    ...
    function inlinecount(value, index) {
        ...
        let token;
        if (v2) {
            if (!utils_1.default.equals(value, index, "allpages")) {
                return;
            }
            token = lexer_1.default.tokenize(value, index, index + 8, "Edm.String", lexer_1.default.TokenType.Literal);
        }
        else {
